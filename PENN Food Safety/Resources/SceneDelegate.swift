...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if  let authState = OBJ_FOR_KEY(FSConstants.UserDefault.accessToken) {
            
            if DataManager.sharedManager.isTokenValids(){ 
                rootVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "FSStartViewController") as! FSStartViewController
            }else  {
